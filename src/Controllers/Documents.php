...
class Documents
{
    ...
    public function __construct($orderId)
    {
        ...
        $documentType = array_key_exists($paymentMethod, $types) ? $types[$paymentMethod] : null;
        $this->documentType = null !== $documentType && $documentType !== 'default' ? $documentType : (isset($_GET['document_type']) ? sanitize_text_field($_GET['document_type']) : DOCUMENT_TYPE);
    }
    ...
    public function createDocument()
    {
        try {
            ...
            if (defined('DOCUMENT_STATUS') && DOCUMENT_STATUS) {
                ...
                // Retrieve PDF file
                $pdf = $this->getInvoicePDF($insertedDocument['document_id']);
     ...
     * @return string|null
     */
    protected function getInvoicePDF($document_id)
    {
        $pdfFilename = null;
        try {
            ...
            $explode = explode('?', $pdfURL['url']);
            $params = array_key_exists(1, $explode) ? $explode[1] : 'h=0&d=0';
            $url = 'https://www.moloni.pt/downloads/index.php?action=getDownload&' . $params . '&e=&i=1';
