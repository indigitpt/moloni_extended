    ...
    <div>
        ...
        <table class="form-table">
            ...
            foreach (WC()->payment_gateways()->payment_gateways() as $gateway):
                ...
                <tr>
                    <th>
                        <label for="document_type_by_payment"><?= __('Tipo de documento (' . $gateway->title . ')') ?></label>
                    </th>
                    <td>
                        <select id="document_type" name='opt[document_type_by_payment][<?php echo $gateway->id ?>]' class='inputOut'>
                            <option value='default' <?= (null === $selected ? 'selected' : '') ?>>
                                <?= __('Tipo de documento definido acima') ?>
                            </option>

                            <option value='invoices' <?= ($selected === 'invoices' ? 'selected' : '') ?>>
                                <?= __('Faturas') ?>
                            </option>

                            <option value='invoiceReceipts' <?= ($selected === 'invoiceReceipts' ? 'selected' : '') ?>>
                                <?= __('Factura/Recibo') ?>
                            </option>

                            <option value='simplifiedInvoices'<?= ($selected === 'simplifiedInvoices' ? 'selected' : '') ?>>
                                <?= __('Factura Simplificada') ?>
                            </option>

                            <option value='billsOfLading' <?= ($selected === 'billsOfLading' ? 'selected' : '') ?>>
                                <?= __('Guia de Transporte') ?>
                            </option>

                            <option value='purchaseOrder' <?= ($selected === 'purchaseOrder' ? 'selected' : '') ?>>
                                <?= __('Nota de Encomenda') ?>
                            </option>

                            <option value='estimates' <?= ($selected === 'estimates' ? 'selected' : '') ?>>
                                <?= __('Orçamento') ?>
                            </option>
                        </select>
                    </td>
            ...
            <tr>
                <th>
                    <label for="email_send_internal"><?= __('Enviar email interno para:') ?></label>
                </th>
                <td>
                    <textarea id="email_send_internal" name="opt[email_send_internal]" class="inputOut"><?php echo EMAIL_SEND_INTERNAL; ?></textarea>
                    <p class='description'><?= __('Lista de emails a enviar a factura internalmente (separados por virgula ",")') ?></p>
                </td>
            </tr>
